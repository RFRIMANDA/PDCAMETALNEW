    ...
    <div class="card">
        <div class="card-body">
            ...
            <form method="GET" action="{{ route('riskregister.tablerisk', $id) }}">
                <div class="mb-3 row">
                    <label class="col-sm-2 col-form-label"><strong>Cari Issue:</strong></label>
                    <div class="col-sm-7">
                ...
                <div class="mb-3 row">
                    <label class="col-sm-2 col-form-label"><strong>Cari Target PIC:</strong></label>
                    <div class="col-sm-7">
                        <div class="input-group">
                            <select name="targetpic" class="form-control">
                                <option value="">Pilih Target PIC</option>
                                @foreach($users as $user)
                                    <option value="{{ $user->nama_user }}" {{ request('targetpic') == $user->nama_user ? 'selected' : '' }}>
                                        {{ $user->nama_user }}
                                    </option>
                                @endforeach
                            </select>
                            <button type="submit" class="btn btn-primary">Cari</button>
                            <a href="{{ route('riskregister.tablerisk', $id) }}" class="btn btn-secondary">Reset</a>
                        </div>
                    </div>
                </div>

                @if(auth()->user()->role === 'admin' || auth()->user()->role === 'supervisor' || auth()->user()->role === 'manager' || auth()->user()->role === 'manajemen' )
                    <a href="{{ route('riskregister.create', $id) }}" class="btn btn-success btn-sm" title="Create List Register">
